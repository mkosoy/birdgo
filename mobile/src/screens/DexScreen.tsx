...
function coordinateLabel(latitude?: number, longitude?: number): string | undefined {
  if (typeof latitude !== "number" || !Number.isFinite(latitude)
    || typeof longitude !== "number" || !Number.isFinite(longitude)) return undefined;
  return `${latitude.toFixed(2)}, ${longitude.toFixed(2)}`;
}

function captureName(capture: { commonName?: unknown }): string {
  return typeof capture.commonName === "string" && capture.commonName.trim()
    ? capture.commonName.trim()
    : "Unidentified bird";
}
...
export function DexScreen() {
  ...
  const capturedNames = new Set(captures.map((capture) => captureName(capture).toLowerCase()).filter((name) => name !== "unidentified bird"));
